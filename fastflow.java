...
public class fastflow
{
    ...
    private static int [] flow, capa, now; // E
    ...
    public static void main (String [] args) throws IOException {
        ...
        eadj = new int [2 * M]; elast = new int [N];
        eprev = new int [2 * M]; eidx = 0;
        Arrays.fill (elast, -1);
        
        flow = new int [2 * M]; capa = new int [2 * M]; now = new int [2 * M];
        level = new int [N];
        
        for (int i = 0; i < M; i++)
            add_edge (in.nextInt () - 1, in.nextInt () - 1, in.nextInt ());
            
        out.println (dinic (0, N - 1));
        out.close ();
        System.exit (0);
    }
